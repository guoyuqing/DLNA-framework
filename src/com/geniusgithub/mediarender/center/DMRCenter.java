...
import com.geniusgithub.mediarender.music.MusicService;
...
public class DMRCenter implements ActionReflectionListener, IDMRAction {
	...
	public void onRenderAvTransport(String value, String data) {
		...
		DlnaMediaModel mediaInfo = DlnaMediaModelFactory
				.createFromMetaData(data);
	...
	private void delayToPlayMusic(DlnaMediaModel mediaInfo) {
		if (mediaInfo != null) {
			...
			Message msg = mHandler
					.obtainMessage(MSG_START_MUSICPLAY, mediaInfo);
	...
	private void delayToStop() {
		clearDelayMsg();
		mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_SEND_STOPCMD),
				DELAYTIME);
	}
	...
	private Handler mHandler = new Handler() {
		...
		public void dispatchMessage(Message msg) {
			...
			try {
				...
				e.printStackTrace();
				log.e("DMRCenter transdel msg catch Exception!!! msgID = "
						+ msg.what);
			}
	...
	private void startPlayPicture(DlnaMediaModel mediaInfo) {
		...
		intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK
				| Intent.FLAG_ACTIVITY_SINGLE_TOP);
		mContext.startActivity(intent);
		System.out.println("aaaaaaaaaaaaaaaaaaa---------ImageActivity");
		RenderApplication.closeMusic();
		RenderApplication.closeVideo();
	}
