...
public class DLNAGenaEventBrocastFactory {
	...
	public void registerBrocast() {
		if (mReceiver == null) {
			...
			mContext.registerReceiver(mReceiver, new IntentFilter(
					PlatinumReflection.RENDERER_TOCONTRPOINT_CMD_INTENT_NAME));
		}
	}
	...
	public static void sendDurationEvent(Context context, int duration) {
		if (duration != 0) {
			...
			setintent
					.putExtra(
							PlatinumReflection.GET_RENDERER_TOCONTRPOINT_CMD,
							PlatinumReflection.MEDIA_RENDER_TOCONTRPOINT_SET_MEDIA_DURATION);
			setintent.putExtra(PlatinumReflection.GET_PARAM_MEDIA_DURATION,
	...
	public static void sendSeekEvent(Context context, int time) {
		if (time != 0) {
			...
			setintent.putExtra(PlatinumReflection.GET_PARAM_MEDIA_POSITION,
