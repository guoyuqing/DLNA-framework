...
public class MediaControlBrocastReceiver extends BroadcastReceiver {
	...
	public void setMediaControlListener(
			MediaControlBrocastFactory.IMediaControlListener listener) {
	...
	private void TransdelControlCommand(Intent intent) {
		...
		String action = intent.getAction();
		if (action
				.equalsIgnoreCase(MediaControlBrocastFactory.MEDIA_RENDERER_CMD_PLAY)) {
			mMediaControlListener.onPlayCommand();
		} else if (action
				.equalsIgnoreCase(MediaControlBrocastFactory.MEDIA_RENDERER_CMD_PAUSE)) {
			mMediaControlListener.onPauseCommand();
			System.out.println("pause::::::::::::::::::::"+action);
		} else if (action
				.equalsIgnoreCase(MediaControlBrocastFactory.MEDIA_RENDERER_CMD_STOP)) {
			mMediaControlListener.onStopCommand();
		} else if (action
				.equalsIgnoreCase(MediaControlBrocastFactory.MEDIA_RENDERER_CMD_SEEKPS)) {
			time = intent.getIntExtra(
					MediaControlBrocastFactory.PARAM_CMD_SEEKPS, 0);
